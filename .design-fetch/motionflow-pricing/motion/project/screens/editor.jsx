...
const EditorScreen = ({ onNav, onContinue, empty = false, paywallOpen: paywallOpenProp = false, onSignIn, onUpgrade }) => {
  const f = useFrame();
  const [playing, setPlaying] = useState(true);
  const [paywallOpen, setPaywallOpen] = useState(paywallOpenProp);
  const [paywallTrigger, setPaywallTrigger] = useState("generate");
  const [credits] = useState(3);
  const triggerPaywall = (kind) => { setPaywallTrigger(kind); setPaywallOpen(true); };
  const [selected, setSelected] = useState(2);
  const [time, setTime] = useState(12.4);
  const [leftTab, setLeftTab] = useState(empty ? "scripts" : "scenes"); // null = collapsed
  const [isEmpty, setIsEmpty] = useState(empty);

  // Track which inputs have been filled — used by the ReadyPanel checklist.
  // In empty mode all start false; once generated, everything is considered ready.
  const [filled, setFilled] = useState(() => empty
    ? { script: false, assets: false, voice: false, music: false }
    : { script: true,  assets: true,  voice: true,  music: true });
  const readyCount = Object.values(filled).filter(Boolean).length;
  const totalCount = 4;
  const canGenerate = filled.script && filled.assets; // script + assets are required; voice/music optional
    ...
    <AppChrome
      ...
      right={
        <>
          ...
          <button onClick={()=>triggerPaywall("generate")} style={{
          ...
          <Button variant="ghost" size="sm" icon={<IconShare size={12}/>} onClick={()=>triggerPaywall("save")}>Share</Button>
          <GenerateButton onClick={()=>triggerPaywall("generate")}/>
          <Button variant="ghost" size="sm" onClick={onSignIn || onContinue}>Sign in</Button>
        </>
      }>

      <div style={{ display: "grid", gridTemplateRows: "1fr auto", height: "100%" }}>
       <div style={{ display: "grid", gridTemplateColumns: `280px 1fr 300px`, minHeight: 0 }}>
        ...
        <aside style={{
          ...
          {/* Accordion */}
          <div style={{ flex: 1, overflow: "auto" }}>          {!isEmpty && (
<AccordionItem k="scenes" label="Scenes" icon={<IconLibrary size={13}/>} active={leftTab} onToggle={setLeftTab}>
          ...
          <AccordionItem k="scripts" label="Script" icon={<IconType size={13}/>} active={leftTab} onToggle={setLeftTab} badge={isEmpty ? <ReadyBadge done={filled.script}/> : null}>
              ...
              {isEmpty ? (
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  <textarea
                    ...
                    onChange={(e)=>setFilled(prev => ({...prev, script: e.target.value.trim().length > 5}))}
                    style={{
          ...
          <AccordionItem k="assets" label="Assets" icon={<IconImage size={13}/>} active={leftTab} onToggle={setLeftTab} badge={isEmpty ? <ReadyBadge done={filled.assets}/> : null}>
              ...
              <div onClick={()=>isEmpty && setFilled(prev => ({...prev, assets: true}))} style={{
                marginBottom: 12, padding: isEmpty ? "22px 10px" : "14px 10px", borderRadius: 10,
                border: `1px dashed ${isEmpty ? (filled.assets ? "rgba(166,240,189,0.45)" : "rgba(122,162,255,0.35)") : "var(--line-2)"}`,
                background: isEmpty ? (filled.assets ? "rgba(166,240,189,0.05)" : "rgba(122,162,255,0.04)") : "rgba(255,255,255,0.015)",
                textAlign: "center", cursor: isEmpty ? "pointer" : "default"
              }}>
                {isEmpty && (
                  <div style={{ width: 28, height: 28, borderRadius: 8, background: filled.assets ? "rgba(166,240,189,0.10)" : "rgba(122,162,255,0.10)", border: `1px solid ${filled.assets ? "rgba(166,240,189,0.40)" : "rgba(122,162,255,0.30)"}`, display: "grid", placeItems: "center", margin: "0 auto 8px", color: filled.assets ? "#A6F0BD" : "#7AA2FF" }}>
                    <IconUpload size={12}/>
                  </div>
                )}
                <div style={{ fontSize: 12, color: "var(--ink-1)", fontWeight: 500 }}>{filled.assets && isEmpty ? "8 files added" : "Drop screenshots"}</div>
          ...
          <AccordionItem k="voiceover" label="Voiceover" icon={<IconMic size={13}/>} active={leftTab} onToggle={setLeftTab} badge={isEmpty ? <ReadyBadge done={filled.voice}/> : null} hint={isEmpty ? "OPTIONAL" : null}>
              ...
              <button onClick={()=>isEmpty && setFilled(prev => ({...prev, voice: true}))} style={{
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {[
                  ...
                  <button key={i} onClick={()=>isEmpty && setFilled(prev => ({...prev, voice: true}))} style={{
          ...
          <AccordionItem k="music" label="Music" icon={<IconMusic size={13}/>} active={leftTab} onToggle={setLeftTab} badge={isEmpty ? <ReadyBadge done={filled.music}/> : null} hint={isEmpty ? "OPTIONAL" : null}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {[
                  ...
                  <button key={i} onClick={()=>isEmpty && setFilled(prev => ({...prev, music: true}))} style={{
                    display: "flex", alignItems: "center", gap: 10,
                    padding: "10px 12px", borderRadius: 10, textAlign: "left",
                    background: "rgba(255,255,255,0.025)", border: "1px solid var(--line)",
                    cursor: "pointer", fontFamily: "inherit"
          ...
          {/* Sticky ready-to-generate panel */}
          <ReadyPanel
            filled={filled}
            setFilled={setFilled}
            ...
            onGenerate={()=>triggerPaywall("generate")}
            isEmpty={isEmpty}
            onJump={(k)=>setLeftTab(k)}
          />
        </aside>
...
// will be assigned at end of file
setTimeout(() => { if (typeof GenerateButton !== "undefined") window.GenerateButton = GenerateButton; }, 0);
...
const AccordionItem = ({ k, label, icon, active, onToggle, children, badge, dim, hint }) => {
  const open = active === k;
  return (
    <div style={{ borderBottom: "1px solid var(--line)", opacity: dim ? 0.55 : 1, transition: "opacity 200ms" }}>
      <button
        onClick={() => onToggle(open ? null : k)}
        style={{
          display: "flex", alignItems: "center", gap: 10,
          width: "100%", padding: "13px 14px",
          background: open ? "rgba(122,162,255,0.04)" : "transparent",
          border: "none", borderLeft: `2px solid ${open ? "#7AA2FF" : "transparent"}`,
          color: open ? "var(--ink-0)" : "var(--ink-2)",
          fontFamily: "inherit", fontSize: 12.5, fontWeight: 500,
          letterSpacing: "-0.005em", textAlign: "left", cursor: "pointer",
          transition: "background 160ms, color 160ms, border-color 160ms"
        }}>
        <span style={{ display: "grid", placeItems: "center", color: open ? "#7AA2FF" : "var(--ink-3)", transition: "color 160ms" }}>{icon}</span>
        <span style={{ flex: 1, display: "flex", alignItems: "center", gap: 8 }}>
          {label}
          {hint && <span className="mf-mono" style={{ fontSize: 9, color: "var(--ink-4)", letterSpacing: "0.08em", fontWeight: 400 }}>{hint}</span>}
        </span>
        {badge}
        <IconChevron size={13} style={{
          color: "var(--ink-3)",
...
const ReadyBadge = ({ done }) => (
  done ? (
    <span style={{
      width: 18, height: 18, borderRadius: "50%",
      background: "rgba(166,240,189,0.12)", border: "1px solid rgba(166,240,189,0.45)",
      color: "#A6F0BD", display: "grid", placeItems: "center", flexShrink: 0,
    }}>
      <svg width="10" height="10" viewBox="0 0 12 12" fill="none">
        <path d="M2.5 6.2L4.8 8.5L9.5 3.8" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
      </svg>
    </span>
  ) : (
    <span className="mf-mono" style={{
...
const ReadyPanel = ({ filled, setFilled, canGenerate, readyCount, totalCount, onGenerate, isEmpty, onJump }) => {
  const items = [
    { k: "scripts", label: "Script",    required: true,  icon: <IconType size={11}/>  },
    { k: "assets",  label: "Assets",    required: true,  icon: <IconImage size={11}/> },
    { k: "voiceover", label: "Voiceover", required: false, icon: <IconMic size={11}/> },
    { k: "music",   label: "Music",     required: false, icon: <IconMusic size={11}/> },
  ];
  return (
    <div style={{
      flexShrink: 0,
      borderTop: "1px solid var(--line)",
      background: "linear-gradient(180deg, rgba(8,9,13,0.6), rgba(8,9,13,0.95))",
      padding: "14px 14px 16px",
    }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 10 }}>
        <span className="mf-eyebrow">{isEmpty ? "READY TO GENERATE" : "PROJECT INPUTS"}</span>
        <span className="mf-mono" style={{ fontSize: 9.5, color: canGenerate ? "#A6F0BD" : "var(--ink-3)", letterSpacing: "0.08em" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 4, marginBottom: 12 }}>
        {items.map(it => {
          const done = filled[it.k === "scripts" ? "script" : it.k === "voiceover" ? "voice" : it.k];
          return (
            <button key={it.k} onClick={()=>onJump(it.k)} style={{
            ...
            onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
              <ReadyBadge done={done}/>
              <span style={{ color: done ? "var(--ink-3)" : "var(--ink-3)", display: "grid", placeItems: "center" }}>{it.icon}</span>
              <span style={{ flex: 1, textDecoration: done ? "line-through" : "none", textDecorationColor: "rgba(166,240,189,0.3)" }}>{it.label}</span>
              {!it.required && (
                <span className="mf-mono" style={{ fontSize: 9, color: "var(--ink-4)", letterSpacing: "0.06em" }}>OPTIONAL</span>
              )}
            </button>
          );
        })}
      ...
      <button
        ...
        style={{
          ...
          border: canGenerate ? "1px solid rgba(167,139,250,0.45)" : "1px solid var(--line)",
          background: canGenerate
            ? "linear-gradient(135deg, #7AA2FF 0%, #A78BFA 55%, #67E8F9 100%)"
            : "rgba(255,255,255,0.03)",
          color: canGenerate ? "#0B0C10" : "var(--ink-4)",
          fontSize: 13, fontWeight: 600, fontFamily: "inherit",
          letterSpacing: "-0.005em",
          cursor: canGenerate ? "pointer" : "not-allowed",
          display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
          boxShadow: canGenerate
            ? "0 6px 20px rgba(122,162,255,0.30), inset 0 1px 0 rgba(255,255,255,0.20)"
            : "none",
          transition: "transform 160ms, box-shadow 160ms",
        }}>
        <IconWand size={13}/>
        <span>{isEmpty ? "Generate scenes" : "Re-generate"}</span>
        <span className="mf-mono" style={{
          fontSize: 9.5, letterSpacing: "0.06em",
          padding: "2px 5px", borderRadius: 4,
          background: canGenerate ? "rgba(11,12,16,0.18)" : "rgba(255,255,255,0.04)",
          border: canGenerate ? "1px solid rgba(11,12,16,0.20)" : "1px solid var(--line)",
          color: canGenerate ? "rgba(11,12,16,0.75)" : "var(--ink-4)",
        }}>⌘ ⏎</span>
      </button>

      {isEmpty && !canGenerate && (
        <div className="mf-mono" style={{ marginTop: 8, fontSize: 9.5, color: "var(--ink-4)", letterSpacing: "0.06em", textAlign: "center" }}>
          ADD A SCRIPT AND ASSETS TO CONTINUE
        </div>
      )}
    </div>
  );
